...
export function extractJavaScript(content: string, file: string, evidenceId: string): ExtractionResult {
  ...
  try {
    ...
    errors.push(`Failed to parse JavaScript: ${error}`)
  }
...
const handlers: Partial<Record<ts.SyntaxKind, Handler>> = {
  ...
  [ts.SyntaxKind.FunctionExpression]: (n, eid, f, s) => extractFunctionExpression(n as ts.FunctionExpression, eid, f, s),
}
...
function extractFunctionDeclaration(node: ts.FunctionDeclaration, evidenceId: string, file: string, signals: Signal[]): void {
  ...
  signals.push(createSignal({
    ...
    tags: ["function", name, isAsync ? "async" : "", isExport ? "export" : ""].filter(Boolean),
    confidence: 0.95,
    lineStart: node.getStart(),
    lineEnd: node.end,
  }))
...
function extractClassDeclaration(node: ts.ClassDeclaration, evidenceId: string, file: string, signals: Signal[]): void {
  ...
  signals.push(createSignal({
    ...
    tags: ["class", name, isExport ? "export" : "", ...heritage].filter(Boolean),
    confidence: 0.95,
    lineStart: node.getStart(),
    lineEnd: node.end,
  }))
...
function extractPropertyDeclaration(node: ts.PropertyDeclaration, evidenceId: string, file: string, signals: Signal[]): void {
  ...
  signals.push(createSignal({
    ...
    file,
    name,
    text: `${isStatic ? "static " : ""}${name}`,
    tags: ["property", name, isStatic ? "static" : ""].filter(Boolean),
    confidence: 0.85,
    lineStart: node.getStart(),
    lineEnd: node.end,
  }))
...
function extractFunctionExpression(node: ts.FunctionExpression, evidenceId: string, file: string, signals: Signal[]): void {
  const parent = node.parent
